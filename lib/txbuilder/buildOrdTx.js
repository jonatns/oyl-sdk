...
function buildOrdTx(psbtTx, segwitUtxos, allUtxos, taprootSigner, segwitSigner, segwitAddress, toAddress, metaOutputValue, inscriptionId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const fee = yield psbtTx.calNetworkFee();
        let feeUtxo;
        ...
        if (remainingUnspent - (yield psbtTx.calNetworkFee()) >= constants_1.UTXO_DUST) {
            psbtTx.getChangeOutput().value =
                remainingUnspent - (yield psbtTx.calNetworkFee());
        }
        else {
            psbtTx.removeChangeOutput();
        }
        const psbt = yield psbtTx.createSignedPsbt(segwitSigner, taprootSigner);
        psbtTx.dumpTx(psbt);
        return psbt;
    });
}
