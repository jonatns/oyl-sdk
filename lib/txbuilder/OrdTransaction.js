...
class OrdTransaction {
    constructor(keyring, network, pubkey, addressType, feeRate) {
        this.inputs = [];
        this.outputs = [];
                ...
                if (script && !isSigned) {
                    ...
                    if (this.address === address) {
                        ...
                        if (this.addressType === "P2TR" && !v.tapInternalKey) {
                            v.tapInternalKey = (0, exports.toXOnly)(Buffer.from(this.pubkey, 'hex'));
                        }
                    }
                }
            });
            psbt = yield this.keyring.signTransaction(psbt, toSignInputs);
