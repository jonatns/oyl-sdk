...
class PSBTTransaction {
     ...
     */
    formatOptionsToSignInputs = async (psbt) => {
        let toSignInputs = [];
    ...
    async signInputs(psbt, toSignInputs) {
        try {
            const taprootInputs = [];
            const segwitInputs = [];
    ...
    async signPsbt(psbt) {
        try {
            const { psbt: formattedPsbt, toSignInputs, } = await this.formatOptionsToSignInputs(psbt);
