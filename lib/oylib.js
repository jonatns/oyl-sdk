...
class WalletUtils {
    ...
    async getMetaBalance(address) {
        ...
        const usdValue = await transactions_1.transactions.convertUsdValue(amount);
    ...
    async getInscriptions(address) {
        const artifacts = await wallet_1.bord.getInscriptionsByAddr(address);
    ...
    async getUtxosArtifacts(address) {
        const utxos = await transactions_1.transactions.getUnspentOutputs(address);
        const inscriptions = await this.getInscriptions(address);
        const utxoArtifacts = await transactions_1.transactions.getMetaUtxos(utxos.unspent_outputs, inscriptions);
