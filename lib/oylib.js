...
class Oyl {
    ...
    async sendCollectibleEstimate({ spendAddress, altSpendAddress, toAddress, feeRate, }) {
        const addressType = (0, transactions_1.getAddressType)(toAddress);
        if (utils_1.addressTypeMap[addressType] === 'p2pkh') {
            throw new Error('Sending bitcoin to legacy address is not supported');
        }
    ...
    async sendBR20Estimate({ toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, token = 'estimate', amount = 1, }) {
        const addressType = (0, transactions_1.getAddressType)(toAddress);
        if (utils_1.addressTypeMap[addressType] === 'p2pkh') {
            throw new Error('Sending bitcoin to legacy address is not supported');
        }
        ...
        const sendTxSize = (0, utils_1.calculateTaprootTxSize)(2, 0, 2);
        let fee = sendTxSize * feeRate < 250 ? 250 : sendTxSize * feeRate;
        const { fee: commitTxFee, utxosUsedForFees } = await this.inscriptionCommitTx({
            content,
            signer,
            spendAddress,
            spendPubKey,
