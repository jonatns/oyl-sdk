...
class Oyl {
    ...
    async sendBRC20({ fromAddress, fromPubKey, toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, token, amount, }) {
        ...
        try {
            ...
            const { signedPsbt: taprootTransferRawPsbt } = await signer.signAllTaprootInputs({
    ...
    async inscriptionTransfer({ commitChangeUtxoId, toAddress, fromPubKey, spendPubKey, altSpendPubKey, usingAlt, spendAddress, fee, }) {
        const psbt = new bitcoin.Psbt({ network: this.network });
