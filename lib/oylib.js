...
class Wallet {
    ...
    createOrdPsbtTx({ publicKey, fromAddress, toAddress, changeAddress, txFee, segwitAddress, segwitPubKey, inscriptionId, mnemonic, }) {
        return __awaiter(this, void 0, void 0, function* () {
            const segwitKeyring = new hdKeyring_1.HdKeyring({
                mnemonic: mnemonic,
                hdPath: "m/49'/0'/0'/0",
                activeIndexes: [0],
            });
            const taprootKeyring = new hdKeyring_1.HdKeyring({
                mnemonic: mnemonic,
                hdPath: "m/86'/0'/0'/0",
                activeIndexes: [0],
            });
            const segwitSigner = yield segwitKeyring.fetchKeyPair(segwitPubKey);
            const signer = yield taprootKeyring.fetchKeyPair(publicKey);
            ...
            const segwitUtxos = yield this.getUtxosArtifacts({ address: segwitAddress });
            const feeRate = txFee;
            ...
            const psbtTx = new txbuilder_1.PSBTTransaction(signer, segwitPubKey, segwitAddressType, fromAddress, publicKey, addressType, feeRate);
            psbtTx.setChangeAddress(changeAddress);
            const finalizedPsbt = yield (0, txbuilder_1.buildOrdTx)(psbtTx, segwitUtxos, allUtxos, signer, segwitSigner, segwitAddress, toAddress, metaOutputValue, inscriptionId);
