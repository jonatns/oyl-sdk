        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (addressType === 1) {
                ...
                const processedTxns = lastTenTxns.map((tx) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const symbols = [];
                    try {
                        for (var _d = true, _e = __asyncValues(inscriptionsOnTx.entries()), _f; _f = yield _e.next(), _a = _f.done, !_a;) {
                            _c = _f.value;
                            _d = false;
                            try {
                                const [index, inscription] = _c;
                                if (inscription.inscriptions.length > 0) {
                                    if (inscription.inscriptions[index]) {
                                        const inscriptionsOnTxContent = yield this.ordRpc.getInscriptionContent(inscription.inscriptions[index]);
                                        const inscriptionDetails = yield this.ordRpc.getInscriptionById(inscription.inscriptions[index]);
