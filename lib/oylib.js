...
class Oyl {
    ...
    async createBtcTx({ toAddress, spendPubKey, feeRate, amount, network, spendUtxos, spendAddress, altSpendPubKey, altSpendUtxos, fee, }) {
        ...
        let { psbt: updatedPsbt } = await (0, utils_1.addBtcUtxo)({
            spendUtxos,
            psbt: psbt,
            toAddress,
            amount: amount,
    ...
    async inscriptionCommitTx({ content, spendAddress, spendPubKey, signer, altSpendPubKey, altSpendAddress, feeRate, fee, }) {
        ...
        let amountNeededForInscribe = fee
            ...
            : Number(feeForCommit) + Number(feeForReveal) + utils_1.inscriptionSats;
    ...
    async inscriptionRevealTx({ receiverAddress, signer, content, feeRate, commitTxId, fee, }) {
        const revealTxSize = (0, utils_1.calculateTaprootTxSize)(1, 0, 1);
        ...
        const txData = tapscript_1.Tx.create({
            ...
            vout: [
                {
                    value: revealSats - feeForReveal - utils_1.inscriptionSats,
        ...
        const inscriptionTx64 = tapscript_1.Tx.encode(txData).base64;
    ...
    async sendBRC20({ fromAddress, fromPubKey, toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, token, amount, }) {
        ...
        try {
            ...
            const revealFee = bitcoin.Psbt.fromBase64(revealTx, { network: this.network })
                .extractTransaction()
                .virtualSize() * feeRate;
            ...
            const revealTxId = await this.sandshrewBtcClient.bitcoindRpc.sendRawTransaction(revealTxHex);
            if (!revealTxId) {
            ...
            for (const tx of feeData) {
                ...
                totalSatsPerVByte += Number(((tx.fees['base'] * 10 ** 8) / (tx.weight / 4)).toFixed(2));
            }
            return {
                txId: sentPsbtTxId,
    ...
    async inscriptionSendTx({ toAddress, fromPubKey, spendPubKey, spendAddress, altSpendAddress, altSpendPubKey, feeRate, utxoId, utxosUsedForFees, fee, }) {
        if (!feeRate) {
            feeRate = (await this.esploraRpc.getFeeEstimates())['1'];
        }
    ...
    async runeSendTx({ runeId, fromAddress, toAddress, amount, spendAddress, spendPubKey, altSpendPubKey, altSpendAddress, feeRate, }) {
        let usingAlt = false;
        let spendUtxos;
        let altSpendUtxos;
        spendUtxos = await this.getUtxosArtifacts({
