...
class Oyl {
    ...
    async inscriptionCommitTx({ content, spendAddress, spendPubKey, signer, altSpendPubKey, altSpendAddress, feeRate, fee, }) {
        ...
        if (utxosToPayFee?.selectedUtxos.length > 1) {
            ...
            utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(spendUtxos, newAmountNeededForInscribe);
            ...
            if (utxosToPayFee?.selectedUtxos.length > 1) {
                ...
                const newAltAmountNeededForInscribe = fee
                    ...
                    : Number(txSize * feeRate < 250 ? 250 : txSize * feeRate) +
                        Number(feeForReveal) +
                        utils_1.inscriptionSats;
                utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(filteredAltUtxos, newAltAmountNeededForInscribe);
                if (!utxosToPayFee) {
                    throw new Error('Insufficient Balance');
                }
        ...
        const changeAmount = feeAmountGathered - (finalFee ? finalFee : amountNeededForInscribe);
