...
class Oyl {
    ...
    getUtxosArtifacts({ address }) {
        return __awaiter(this, void 0, void 0, function* () {
            const { unspent_outputs } = yield this.getUtxos(address, true);
    ...
    sendBtcEstimate({ fromAddress, toAddress, feeRate, amount, altSpendPubKey, spendAddress, spendPubKey, altSpendAddress, }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { fee } = yield this.createBtcTx({
                fromAddress,
                toAddress,
                spendPubKey,
                feeRate,
    ...
    sendRune({ fromAddress, fromPubKey, toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, symbol, amount, }) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const outPutIndex = '2';
                const { commitPsbt } = yield this.runeCommitTx({
                    symbol,
                    amount,
                    spendAddress,
                    spendPubKey,
    ...
    runeCommitTx({ symbol, amount, spendAddress, spendPubKey, signer, altSpendPubKey, altSpendAddress, feeRate, outPutIndex, }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const feeForCommit = commitTxSize * feeRate < 200 ? 200 : commitTxSize * feeRate;
            const amountNeededForInscribe = Number(feeForCommit) + utils_1.inscriptionSats;
            const utxosUsedForFees = [];
            let usingAlt = false;
            let spendUtxos;
            let altSpendUtxos;
            ...
            const pointer = '16' + Buffer.from(outPutIndex, 'hex').toString('hex');
            const data = pointer + Buffer.from('', 'hex').toString('hex');
            const opReturnScript = bitcoin.script.compile(Buffer.from(data, 'hex'));
