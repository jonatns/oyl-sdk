...
class Oyl {
    ...
    getTaprootBalance({ address }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const txns = yield this.esploraRpc.getAddressUtxo(address);
            let confirmedAmount = 0;
            let pendingAmount = 0;
            for (const utxo of txns) {
                const { txid, status, vout } = utxo;
                let inscriptionsOnTx = yield this.ordRpc.getTxOutput(`${txid}:${vout}`);
                if (inscriptionsOnTx.inscriptions.length === 0) {
                    if (status.confirmed)
                        confirmedAmount += utxo.value / 1e8;
                    if (!status.confirmed)
                        pendingAmount += utxo.value / 1e8;
                }
            }
            ...
            if (addressType === 1) {
                ...
                const processedTxns = lastTenTxns.map((tx) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    for (let [index, output] of vout.entries()) {
                        ...
                        const inscription = yield this.ordRpc.getTxOutput(`${txid}:${index}`);
                    ...
                    for (const [index, inscription] of inscriptionsOnTx.entries()) {
                        if (inscription.inscriptions.length > 0) {
                            if (inscription.inscriptions[index]) {
                                const inscriptionContent = yield this.ordRpc.getInscriptionContent(inscription.inscriptions[index]);
                                const { content_type: contentType, inscription_id: inscriptionId, inscription_number: inscriptionNumber, } = yield this.ordRpc.getInscriptionById(inscription.inscriptions[index]);
                                if (contentType.startsWith('image/png')) {
                                    inscriptionType = 'collectible';
                                    const collectibleInscription = {
                                        contentType,
                                ...
                                if (contentType.startsWith('text/plain')) {
                                    try {
                                        ...
                                        console.error('Unable to parse inscription content', inscriptionContent);
                                    }
                                }
                            }
                        }
                    }
                    const blockDelta = currentBlock - (status === null || status === void 0 ? void 0 : status.block_height) + 1;
    ...
    sendBRC20(options) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const { txId } = yield (0, utils_1.inscribe)({
                    content,
