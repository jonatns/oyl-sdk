    ...
    createRevealTxn({ senderAddress, signer, content, feeRate, commitTxId, senderPublicKey, }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            psbt.addInput({
                ...
                witnessUtxo: {
                    value: revealSats,
                    script: Buffer.from('OP_1' + tpubkey),
                },
            });
            psbt.addOutput({
                value: 546,
    ...
    sendBRC20({ signer, senderAddress, receiverAddress, senderPublicKey, payFeesWithSegwit, segwitFeePublicKey, feeRate, token, amount, }) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const { commitPsbt } = yield this.createInscriptionCommitPsbt({
                    content,
                    senderAddress: senderAddress,
                    senderPublicKey: senderPublicKey,
                    signer,
                ...
                const { signedPsbt } = yield this.signAPsbt({
                    payFeesWithSegwit: payFeesWithSegwit,
                    psbt: commitPsbt,
                    signer: signer,
                ...
                const { revealPsbt, tapleaf } = yield this.createRevealTxn({
                    senderAddress,
                    senderPublicKey,
                    signer,
                    content,
                ...
                const { signedPsbt: signedRevealedPsbt } = yield this.signAPsbt({
                    payFeesWithSegwit: false,
                    psbt: revealPsbt,
                    signer,
                    inputAddressType,
                    tapleaf: Buffer.from(tapleaf),
                });
                const { txId: revealTxId } = yield this.pushPsbt({
                ...
                const { rawTx: sentRawTx, txId: sentTxId } = yield this.sendOrdCollectible({
                    senderAddress,
                    receiverAddress,
                    senderPublicKey,
                    payFeesWithSegwit,
