...
class Oyl {
    ...
    async sendBtc({ toAddress, feeRate, amount, altSpendPubKey, spendAddress, spendPubKey, altSpendAddress, signer, }) {
        ...
        const fee = (await this.sandshrewBtcClient.bitcoindRpc.decodePSBT(taprootSigned)).tx
            .vsize * feeRate;
    ...
    async sendRune({ fromAddress, toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, runeId, amount, }) {
        ...
        const { signedPsbt: taprootSigned, raw } = await signer.signAllTaprootInputs({
            rawPsbt: segwitSigned,
            finalize: true,
        });
        ...
        const { signedPsbt: taprootSigned1, raw: raw2 } = await signer.signAllTaprootInputs({
            rawPsbt: segwitSigned1,
            finalize: true,
        });
        const vsize2 = (await this.sandshrewBtcClient.bitcoindRpc.decodeRawTransaction(raw2.extractTransaction().toHex())).vsize;
