...
class Oyl {
    ...
    sendBtc({ to, from, amount, feeRate, publicKey, mnemonic, segwitAddress, segwitPubkey, segwitHdPathWithIndex, taprootHdPathWithIndex, payFeesWithSegwit = true, }) {
        return __awaiter(this, void 0, void 0, function* () {
            const { txId, rawTx } = yield (0, utils_1.createBtcTx)({
                inputAddress: from,
                outputAddress: to,
                amount: amount,
                feeRate: feeRate,
            ...
            const [result] = yield this.sandshrewBtcClient._call('btc_testmempoolaccept', [[rawTx]]);
            if (!result.allowed) {
                throw new Error(result['reject-reason']);
            }
            const broadcastResult = yield this.sandshrewBtcClient._call('btc_sendrawtransaction', [rawTx]);
