...
class Oyl {
    ...
    async inscriptionCommitTx({ content, spendAddress, spendPubKey, signer, altSpendPubKey, altSpendAddress, feeRate, fee, }) {
        const commitTxSize = (0, utils_1.calculateTaprootTxSize)(2, 0, 2);
        ...
        console.log(feeAmountGathered, amountNeededForInscribe, feeForReveal + utils_1.inscriptionSats + feeForCommit);
    ...
    async sendBRC20({ fromAddress, fromPubKey, toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, token, amount, }) {
        ...
        try {
            ...
            // })
            // await delay(5000)
            const feeTxPromise = successTxIds.map((txId) => (0, utils_1.delay)(1)
    ...
    async createOrdCollectibleTx({ inscriptionId, fromAddress, fromPubKey, spendPubKey, spendAddress, toAddress, altSpendAddress, altSpendPubKey, feeRate, fee, }) {
        ...
        let estimate = sendTxSize * feeRate < 250 ? 250 : sendTxSize * feeRate;
        let usingAlt = false;
        let spendUtxos;
        let altSpendUtxos;
        spendUtxos = await this.getUtxosArtifacts({
        ...
        if (utxosToSend?.selectedUtxos.length > 1) {
            ...
            utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(availableUtxos, fee ? fee : estimate);
        }
        if (!utxosToSend) {
