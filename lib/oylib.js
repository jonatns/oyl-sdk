...
const transactions = tslib_1.__importStar(require("./transactions"));
...
class Oyl {
    ...
    constructor(opts = constants_1.defaultNetworkOptions.mainnet) {
        ...
        this.provider = new provider_1.Provider(rpcUrl);
    ...
    async sendBtc({ toAddress, feeRate, amount, altSpendPubKey, spendAddress, spendPubKey, altSpendAddress, signer, }) {
        try {
            ...
            const { psbt } = await this.createBtcTx({
                toAddress,
                spendPubKey,
                feeRate,
                amount,
            ...
            const vsize = (await this.sandshrewBtcClient.bitcoindRpc.decodeRawTransaction(raw.extractTransaction().toHex())).vsize;
            const fee = vsize * feeRate;
            const { psbt: finalRawPsbt } = await this.createBtcTx({
                toAddress,
                spendPubKey,
                feeRate,
                amount,
    ...
    async createBtcTx({ toAddress, amount, feeRate, network, spendAddress, spendPubKey, altSpendPubKey, altSpendAddress, fee, }) {
        const addressType = (0, transactions_1.getAddressType)(toAddress);
        if (utils_1.addressTypeMap[addressType] === 'p2pkh') {
            throw new Error('Sending bitcoin to legacy address is not supported');
        }
        ...
        let utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(spendUtxos, amount + finalFee);
        let usingAlt = false;
        if (utxosToSend?.selectedUtxos.length > 1) {
            ...
            utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(spendUtxos, amount + finalFee);
        }
        if (!utxosToSend) {
            if (!altSpendAddress) {
            ...
            utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(altSpendUtxos, amount + finalFee);
            if (utxosToSend?.selectedUtxos.length > 1) {
                const txSize = (0, utils_1.calculateTaprootTxSize)(utxosToSend.selectedUtxos.length, 0, 2);
                fee = txSize * feeRate < 250 ? 250 : txSize * feeRate;
                utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(spendUtxos, amount + finalFee);
    ...
    async inscriptionCommitTx({ content, spendAddress, spendPubKey, signer, altSpendPubKey, altSpendAddress, feeRate, fee = 0, finalSendFee, }) {
        ...
        let amountNeededForInscribe = fee !== 0
            ? fee + Number(feeForReveal) + utils_1.inscriptionSats + finalSendFee
            : baseEstimate;
    ...
    async sendBRC20({ fromAddress, fromPubKey, toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, token, amount, }) {
        ...
        try {
            ...
            const sendVsize = (await this.sandshrewBtcClient.bitcoindRpc.decodeRawTransaction(rawSend.extractTransaction().toHex())).vsize;
            ...
            const finalSendVsize = (await this.sandshrewBtcClient.bitcoindRpc.decodeRawTransaction(finalRawSend.extractTransaction().toHex())).vsize;
            const finalSendFee = finalSendVsize * feeRate;
            const { commitPsbt, usingAlt, script } = await this.inscriptionCommitTx({
                content,
                spendAddress,
                spendPubKey,
                signer,
            ...
            const commitVsize = (await this.sandshrewBtcClient.bitcoindRpc.decodeRawTransaction(raw.extractTransaction().toHex())).vsize;
            ...
            const revealVsize = (await this.sandshrewBtcClient.bitcoindRpc.decodeRawTransaction(revealRaw.extractTransaction().toHex())).vsize;
            ...
            const { signedPsbt: segwitTransferRawPsbt } = await signer.signAllSegwitInputs({
                rawPsbt: transferRawPsbt,
                finalize: true,
            });
            ...
            const transferVsize = (await this.sandshrewBtcClient.bitcoindRpc.decodeRawTransaction(rawTransferPsbt.extractTransaction().toHex())).vsize;
            ...
            const { signedPsbt: segwitTransferRawPsbt1 } = await signer.signAllSegwitInputs({
                rawPsbt: transferRawPsbt1,
                finalize: true,
            });
            const { signedPsbt: taprootTransferRawPsbt1 } = await signer.signAllTaprootInputs({
    ...
    async inscriptionTransfer({ commitChangeUtxoId, toAddress, spendPubKey, altSpendPubKey, usingAlt, spendAddress, revealTxId, fee, fromPubKey, }) {
        const psbt = new bitcoin.Psbt({ network: this.network });
    ...
    async inscriptionSendTx({ toAddress, fromPubKey, spendPubKey, spendAddress, altSpendAddress, altSpendPubKey, feeRate, fee = 0, }) {
        if (!feeRate) {
            feeRate = (await this.esploraRpc.getFeeEstimates())['1'];
        }
        ...
        spendUtxos = await this.getSpendableUtxos(spendAddress);
        const psbt = new bitcoin.Psbt({ network: this.network });
        let utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(spendUtxos, finalFee);
        if (utxosToPayFee?.selectedUtxos.length > 1) {
            const txSize = (0, utils_1.calculateTaprootTxSize)(utxosToPayFee.selectedUtxos.length, 0, 2);
            const sendTxFee = txSize * feeRate < 250 ? 250 : txSize * feeRate;
            const finalFee = sendTxFee - fee;
            utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(spendUtxos, finalFee);
        }
        if (!utxosToPayFee) {
            if (!altSpendAddress) {
                throw new Error('Insufficient Balance');
            ...
            altSpendUtxos = await this.getSpendableUtxos(altSpendAddress);
            utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(altSpendUtxos, finalFee);
            if (utxosToPayFee?.selectedUtxos.length > 1) {
                const txSize = (0, utils_1.calculateTaprootTxSize)(utxosToPayFee.selectedUtxos.length, 0, 2);
                const sendTxFee = txSize * feeRate < 250 ? 250 : txSize * feeRate;
                const finalFee = sendTxFee - fee;
        ...
        return { sentPsbt: formattedPsbt.toBase64(), usingAlt: usingAlt };
    }
    ...
    async sendCollectibleEstimate({ spendAddress, altSpendAddress, feeRate, }) {
        ...
        if (!utxosToSend) {
            altSpendUtxos = await this.getSpendableUtxos(altSpendAddress);
            utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(altSpendUtxos, fee);
            if (utxosToSend?.selectedUtxos.length > 1) {
                const txSize = (0, utils_1.calculateTaprootTxSize)(utxosToSend.selectedUtxos.length, 0, 2);
                fee = txSize * feeRate < 250 ? 250 : txSize * feeRate;
                utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(altSpendUtxos, fee);
    ...
    async sendRuneEstimate({ spendAddress, altSpendAddress, feeRate, }) {
        ...
        const sendTxSize = (0, utils_1.calculateTaprootTxSize)(2, 0, 4);
        let fee = sendTxSize * feeRate < 250 ? 250 : sendTxSize * feeRate;
        const availableUtxos = await (0, utils_1.filterTaprootUtxos)({
            taprootUtxos: spendUtxos,
        });
        let utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(availableUtxos, fee);
        if (utxosToSend?.selectedUtxos.length > 1) {
            const txSize = (0, utils_1.calculateTaprootTxSize)(1 + utxosToSend.selectedUtxos.length, 0, 4);
            fee = txSize * feeRate < 250 ? 250 : txSize * feeRate;
            utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(availableUtxos, fee);
        }
        if (!utxosToSend) {
    ...
    async sendBrc20Estimate({ spendAddress, altSpendAddress, feeRate, }) {
        ...
        const feeForReveal = revealTxSize * feeRate < 250 ? 250 : revealTxSize * feeRate;
        const amountNeededForInscribe = Number(feeForCommit) + Number(feeForReveal) + Number(sendTxSize) + 546;
        let utxosForCommitandReveal = (0, txbuilder_1.findUtxosToCoverAmount)(spendUtxos, amountNeededForInscribe);
        if (utxosForCommitandReveal?.selectedUtxos.length > 1) {
            ...
            fee = txSize * feeRate < 250 ? 250 : txSize * feeRate;
            utxosForCommitandReveal = (0, txbuilder_1.findUtxosToCoverAmount)(spendUtxos, amountNeededForInscribe);
        }
        if (!utxosForCommitandReveal) {
            altSpendUtxos = await this.getSpendableUtxos(altSpendAddress);
            utxosForCommitandReveal = (0, txbuilder_1.findUtxosToCoverAmount)(altSpendUtxos, amountNeededForInscribe);
            if (utxosForCommitandReveal?.selectedUtxos.length > 1) {
                const txSize = (0, utils_1.calculateTaprootTxSize)(utxosForCommitandReveal.selectedUtxos.length, 0, 2);
                fee = txSize * feeRate < 250 ? 250 : txSize * feeRate;
                utxosForCommitandReveal = (0, txbuilder_1.findUtxosToCoverAmount)(altSpendUtxos, amountNeededForInscribe);
    ...
    async runeSendTx({ runeId, fromAddress, toAddress, amount, spendAddress, spendPubKey, altSpendPubKey, altSpendAddress, feeRate, fee, }) {
        ...
        const txSize = (0, utils_1.calculateTaprootTxSize)(1 + psbt.inputCount, 0, 4);
        ...
        if (utxosToPayFee?.selectedUtxos.length > 1) {
            const txSize = (0, utils_1.calculateTaprootTxSize)(utxosToPayFee.selectedUtxos.length + psbt.inputCount, 0, 4);
        ...
        psbt.addOutput({
            address: spendAddress,
            value: changeAmount + (useableUtxos.totalSatoshis - utils_1.inscriptionSats),
        });
