...
class Oyl {
    ...
    async sendRune({ toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, runeId, amount, }) {
        if (!feeRate) {
            feeRate = (await this.esploraRpc.getFeeEstimates())['1'];
        }
        const runeBalances = await (await fetch(`https://testnet.sandshrew.io:8443/rune_balances?address=eq.${spendAddress}`)).json();
    ...
    async runeSendTx({ runeId, toAddress, amount, spendAddress, spendPubKey, altSpendPubKey, altSpendAddress, feeRate, }) {
        const txSize = (0, utils_1.calculateTaprootTxSize)(2, 0, 3);
