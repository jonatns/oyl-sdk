...
const createNewPoolPsbt = async ({ calldata, token0, token0Amount, token1, token1Amount, utxos, feeRate, account, provider, }) => {
    const tokens = [
        { alkaneId: token0, amount: token0Amount },
        { alkaneId: token1, amount: token1Amount },
    ];
    const { utxos: alkanesUtxos, edicts } = await (0, exports.splitAlkaneUtxos)(tokens, utxos);
...
const createNewPool = async ({ calldata, token0, token0Amount, token1, token1Amount, utxos, feeRate, account, signer, provider, }) => {
...
const splitAlkaneUtxos = (tokens, utxos, virtualOut) => {
    const allTokenUtxos = tokens.map((token) => {
        const selected = (0, utxo_1.selectAlkanesUtxos)({
            utxos,
        ...
        return [
            {
                ...
                output: (0, u32_1.u32)(virtualOut ?? 5),
            },
        ];
    });
    const protostone = (0, proto_runestone_upgrade_1.encodeRunestoneProtostone)({
...
const poolPsbt = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, fee = 0, }) => {
    try {
        let alkanesAddress;
        let alkanesPubkey;
        if (account.taproot) {
        ...
        let gatheredUtxos = (0, __1.findXAmountOfSats)(totalSpendableUtxos.utxos, Number(finalFee) + 546);
        let psbt = new bitcoin.Psbt({ network: provider.network });
        if (alkanesUtxos) {
            await (0, __1.addInputUtxosToPsbt)(alkanesUtxos, psbt, account, provider);
        }
        ...
        await (0, __1.addInputUtxosToPsbt)(gatheredUtxos.utxos, psbt, account, provider);
        psbt.addOutput({
        ...
        const alkanesTotalAmount = alkanesUtxos.reduce((acc, utxo) => acc + utxo.satoshis, 0);
        const changeAmount = gatheredUtxos.totalAmount + alkanesTotalAmount - finalFee - 546;
        psbt.addOutput({
