...
const createNewPoolPsbt = async ({ calldata, token0, token0Amount, token1, token1Amount, gatheredUtxos, feeRate, account, provider, }) => {
    ...
    const { utxos, edicts, totalAmount } = await (0, exports.splitAlkaneUtxos)(tokens, account, provider);
...
const splitAlkaneUtxos = async (tokens, account, provider) => {
    let gatheredUtxos = { utxos: [], totalAmount: 0 };
    ...
    gatheredUtxos = {
        ...
        totalAmount: allTokenUtxos.reduce((acc, t) => acc + t.totalSatoshis, 0),
    };
