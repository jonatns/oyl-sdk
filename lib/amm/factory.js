...
const createNewPool = async (calldata, token0, token0Amount, token1, token1Amount, gatheredUtxos, feeRate, account, signer, provider) => {
    ...
    const protostone1 = (0, proto_runestone_upgrade_1.encodeRunestoneProtostone)({
        protostones: [
...
const splitAlkaneUtxos = async (token0, token0Amount, token1, token1Amount, gatheredUtxos, feeRate, account, signer, provider) => {
    let tokenUtxos;
    const [token0Utxos, token1Utxos] = await Promise.all([
        (0, alkanes_1.findAlkaneUtxos)({
            address: account.taproot.address,
