...
exports.etchReveal = exports.etchCommit = exports.mint = exports.send = exports.actualEtchRevealFee = exports.actualEtchCommitFee = exports.actualMintFee = exports.actualSendFee = exports.findRuneUtxos = exports.createEtchReveal = exports.createEtchCommit = exports.createMintPsbt = exports.createSendPsbt = void 0;
...
const createSendPsbt = async ({ gatheredUtxos, account, runeId, provider, inscriptionAddress = account.taproot.address, toAddress, amount, feeRate, fee, }) => {
    try {
        ...
        if (gatheredUtxos.utxos.length > 1) {
            ...
            gatheredUtxos = (0, utils_1.findXAmountOfSats)(gatheredUtxos.utxos, Number(finalFee) + Number(amount));
        }
...
const createMintPsbt = async ({ gatheredUtxos, account, runeId, provider, feeRate, fee, }) => {
    try {
        ...
        if (!gatheredUtxos) {
            const { accountSpendableTotalUtxos } = await (0, utxo_1.accountUtxos)({
                account,
                provider,
            });
            gatheredUtxos = (0, utils_1.findXAmountOfSats)(accountSpendableTotalUtxos, Number(finalFee) + Number(utils_1.inscriptionSats));
        }
        gatheredUtxos = (0, utils_1.findXAmountOfSats)(gatheredUtxos.utxos, Number(finalFee) + Number(utils_1.inscriptionSats));
        let psbt = new bitcoin.Psbt({ network: provider.network });
...
const createEtchReveal = async ({ symbol, cap, premine, perMintAmount, turbo, divisibility, runeName, receiverAddress, script, feeRate, tweakedTaprootKeyPair, taprootKeyPair, provider, fee = 0, commitTxId, }) => {
    try {
        if (!feeRate) {
