...
const getRuneOutpoints = async ({ address, provider, runeId, }) => {
    ...
    console.log("runeUtxosOutpoints", runeUtxosOutpoints);
...
const findRuneUtxos = async ({ address, greatestToLeast, provider, runeId, targetNumberOfRunes, }) => {
    const runeUtxos = [];
    const runeUtxoOutpoints2 = await provider.api.getRuneOutpoints({
        address: address,
    });
    const runeUtxoOutpoints = await getRuneOutpoints({
        address: address,
        provider: provider,
        runeId: runeId,
    });
