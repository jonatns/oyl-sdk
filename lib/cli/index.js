...
const btc = tslib_1.__importStar(require("../btc/btc"));
const brc20 = tslib_1.__importStar(require("../brc20/brc20"));
const collectible = tslib_1.__importStar(require("../collectible/collectible"));
const rune = tslib_1.__importStar(require("../rune/rune"));
const account_1 = require("../account/account");
const bitcoin = tslib_1.__importStar(require("bitcoinjs-lib"));
const provider_1 = require("../provider/provider");
const index_1 = require("../signer/index");
...
const addressBRC20UtxosAvailable = new commander_1.Command('addressBRC20Utxos')
    .description('Returns all BRC20 utxos')
    .requiredOption('-p, --provider <provider>', 'provider to use when querying the network for utxos')
    .requiredOption('-a, --address <address>', 'address you want to get utxos for')
    .action(async (options) => {
    const provider = defaultProvider[options.provider];
    console.log(await (0, utxo_1.addressBRC20Utxos)({
...
const btcSend = new commander_1.Command('send')
  ...
  */
    .action(async (options) => {
    const provider = defaultProvider[options.provider];
    const signer = new index_1.Signer(provider.network, {
...
const marketPlaceBuy = new commander_1.Command('buy')
    ...
    */
    ...
    switch (options.assetType) {
        case 'BRC20':
            options.assetType = interface_1.AssetType.BRC20;
            quotes = await provider.api.getBrc20Offers({
                ticker: options.ticker,
            });
            break;
        case 'RUNES':
            options.assetType = interface_1.AssetType.RUNES;
            quotes = await provider.api.getRuneOffers({
                ticker: options.ticker,
            });
            break;
        case 'COLLECTIBLE':
            options.assetType = interface_1.AssetType.COLLECTIBLE;
            break;
        default:
            throw new errors_1.OylTransactionError(Error('Incorrect asset type'));
    }
    const marketplace = new trade_1.Trade({
        provider: provider,
