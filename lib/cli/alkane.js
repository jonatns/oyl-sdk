...
exports.alkaneUnwrapBtc = exports.alkaneWrapBtc = exports.subfrostWrapAddress = exports.merkleClaim = exports.initMerkleRoot = exports.alkanePreviewRemoveLiquidity = exports.alkaneGetAllPoolsDetails = exports.alkaneSimulate = exports.alkaneAddLiquidity = exports.alkaneCreatePool = exports.alkaneMultiSend = exports.alkaneSend = exports.alkaneSwap = exports.alkaneRemoveLiquidity = exports.alkaneExecute = exports.alkaneTokenDeploy = exports.alkaneSpendCommit = exports.alkaneContractDeploy = exports.alkanesTrace = exports.AlkanesCommand = void 0;
...
const alkanes_2 = require("../rpclient/alkanes");
...
const merkle_1 = require("../alkanes/merkle");
/* @dev example call
...
class AlkanesCommand extends commander_1.Command {
    ...
    action(fn) {
        ...
        return super.action(async (options) => {
            alkanes_2.metashrew.set(options.metashrewRpcUrl || null);
...
exports.alkaneContractDeploy = new AlkanesCommand('new-contract')
    ...
    console.log(await (0, token_1.inscribePayload)({
        protostone,
        payload,
        utxos: accountUtxos,
...
exports.alkaneSpendCommit = new AlkanesCommand('spend-commit')
    .requiredOption('-txid, --commitTxId <commitTxId>', 'The transaction id of the commit to spend.')
    .requiredOption('-c, --contract <contract>', 'Relative path to contract wasm file to deploy (e.g., "../alkanes/free_mint.wasm")')
    .requiredOption('-data, --calldata <calldata>', 'op code + params to be used when deploying a contracts', (value, previous) => {
    const items = value.split(',');
    return previous ? previous.concat(items) : items;
    ...
    const encoded = (0, proto_runestone_upgrade_1.encodeRunestoneProtostone)({
        protostones: [
            protostone_1.ProtoStone.message({
                protocolTag: 1n,
                edicts: [],
    ...
    const tweakedTaprootKeyPair = (0, utils_1.tweakSigner)(wallet.signer.taprootKeyPair, {
        network: wallet.provider.network,
    });
    const tweakedPublicKey = tweakedTaprootKeyPair.publicKey;
    const script = Buffer.from((0, alkanes_1.p2tr_ord_reveal)((0, bip371_1.toXOnly)(tweakedPublicKey), [payload]).script);
    console.log(await alkanes.deployReveal({
        payload,
...
exports.alkaneTokenDeploy = new AlkanesCommand('new-token')
    ...
    .option('-amount, --amount-per-mint <amount-per-mint>', 'Amount of tokens minted each time mint is called')
    ...
    .option('-p, --provider <provider>', 'Network provider type (regtest, bitcoin)')
    .option('-args, --arguments <arguments>', 'opcode and params to be used when deploying a contract (e.g. -args "1,2,3"). Note: if using this arg, the values for premine, amount-per-mint, cap, token-name, and token-symbol will be ignored', (value, previous) => {
    const items = value.split(',');
    return previous ? previous.concat(items) : items;
}, [])
    .option('-feeRate, --feeRate <feeRate>', 'fee rate')
    ...
    if (options.arguments.length > 0) {
        ...
        const tokenName = (0, utils_2.packUTF8)(options.tokenName);
        const tokenSymbol = (0, utils_2.packUTF8)(options.tokenSymbol);
...
exports.alkaneExecute = new AlkanesCommand('execute')
    ...
    .option('-a, --alkanes <alkanes>', 'alkanes to spend for transaction', (value, previous) => {
    const items = value.split(',');
    return previous ? previous.concat(items) : items;
}, [])
...
exports.alkaneSwap = new AlkanesCommand('swap')
    ...
    const filteredUtxos = accountUtxos.filter(utxo => {
        const addressKey = (0, utils_2.getAddressKey)(utxo.address);
    ...
    const { utxos: alkanesUtxos } = (0, factory_1.splitAlkaneUtxos)(swapToken, filteredUtxos);
...
exports.alkaneMultiSend = new AlkanesCommand('multi-send')
    .requiredOption('-sends, --sends <sends>', 'sends for protostone', (value, previous) => {
    const items = value.split(',');
    return previous ? previous.concat(items) : items;
}, [])
    .requiredOption('-blk, --block <block>')
    ...
    console.log(await (0, token_1.alkaneMultiSend)({
        utxos: accountUtxos,
        alkaneId: { block: options.block, tx: options.txNum },
        sends,
        account: wallet.account,
...
exports.initMerkleRoot = new AlkanesCommand('init-merkle-root')
    ...
    const fileContent = await fs_extra_1.default.readFile(options.file, 'utf-8');
    const rawRecords = (0, sync_1.parse)(fileContent, {
        columns: true,
    ...
    const leaves = records.map(record => new merkle_1.SchemaMerkleLeaf({
        address: record.address,
        amount: BigInt(record.amount),
    }));
    const leafHashes = leaves.map(leaf => {
        const serializedLeaf = borsh.serialize(merkle_1.leafSchema, leaf);
        return (0, sha2_1.sha256)(serializedLeaf);
    });
    const root = (0, merkle_1.calculateMerkleRoot)(leafHashes);
    const rootFirstHalf = (0, utils_2.readU128LE)(root.slice(0, 16));
    const rootSecondHalf = (0, utils_2.readU128LE)(root.slice(16, 32));
    ...
    if (options.witnessProxy) {
        const payload = {
            body: (0, bytes_1.encipher)(merkleCalldata),
            cursed: false,
            tags: { contentType: '' },
        };
        console.log(await (0, token_1.inscribePayload)({
...
exports.merkleClaim = new AlkanesCommand('merkle-claim')
    ...
    const serializedLeaf = borsh.serialize(merkle_1.leafSchema, leafToClaim);
    const leafHashes = leaves.map(leaf => {
        const serialized = borsh.serialize(merkle_1.leafSchema, leaf);
        return (0, sha2_1.sha256)(serialized);
    });
    const proofHashes = (0, merkle_1.generateProof)(leafHashes, leafIndex);
    const merkleProof = new merkle_1.SchemaMerkleProof({
        leaf: serializedLeaf,
        proofs: proofHashes,
    });
    const witnessData = borsh.serialize(merkle_1.proofSchema, merkleProof);
    const [block, tx] = options.target.split(':');
...
/* @dev example call
 ...
 oyl alkane wrap-address -target "2:1" -inputs "32"
  ...
  First input is the opcode
*/
exports.subfrostWrapAddress = new AlkanesCommand('wrap-address')
    .requiredOption('-target, --target <target>', 'target block:tx for simulation', (value) => {
    const [block, tx] = value.split(':').map((part) => part.trim());
    return { block: block.toString(), tx: tx.toString() };
})
    ...
    console.log(JSON.stringify(await (0, subfrost_1.getWrapAddress)(wallet.provider), null, 2));
});
exports.alkaneWrapBtc = new AlkanesCommand('wrap-btc')
    ...
    const wrapAddress = await (0, subfrost_1.getWrapAddress)(wallet.provider);
    ...
    console.log(await alkanes.wrapBtc({
        protostone,
        utxos: accountUtxos,
        feeRate: wallet.feeRate,
        account: wallet.account,
...
exports.alkaneUnwrapBtc = new AlkanesCommand('unwrap-btc')
    ...
    console.log(await alkanes.unwrapBtc({
        utxos: accountUtxos,
        feeRate: wallet.feeRate,
        account: wallet.account,
        signer: wallet.signer,
