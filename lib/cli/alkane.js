...
exports.alkaneSimulate = exports.alkaneMint = exports.alkaneCreatePool = exports.alkaneSend = exports.alkaneSplit = exports.alkaneSwap = exports.alkaneBurn = exports.alkaneExecute = exports.alkaneTokenDeploy = exports.alkaneContractDeploy = exports.alkanesTrace = void 0;
...
*/
exports.alkaneBurn = new commander_1.Command('burn')
    .requiredOption('-data, --calldata <calldata>', 'op code + params to be called on a contract', (value, previous) => {
    const items = value.split(',');
    return previous ? previous.concat(items) : items;
}, [])
    ...
    const calldata = options.calldata.map((item) => BigInt(item));
    console.log(await (0, pool_1.burn)(calldata, { block: options.block, tx: options.txNum }, BigInt(options.amount), {
        utxos: accountSpendableTotalUtxos,
        totalAmount: accountSpendableTotalBalance,
    }, wallet.feeRate, wallet.account, wallet.signer, wallet.provider));
});
/* @dev example call
...
exports.alkaneSwap = new commander_1.Command('swap')
    ...
    const calldata = options.calldata.map((item) => BigInt(item));
    console.log(await (0, pool_1.swap)(calldata, { block: options.block, tx: options.txNum }, BigInt(options.amount), {
        utxos: accountSpendableTotalUtxos,
        totalAmount: accountSpendableTotalBalance,
    }, wallet.feeRate, wallet.account, wallet.signer, wallet.provider));
});
...
exports.alkaneSplit = new commander_1.Command('split')
    ...
    console.log(await (0, factory_1.splitAlkaneUtxos)(alkaneTokensToSplit, {
        utxos: accountSpendableTotalUtxos,
        totalAmount: accountSpendableTotalBalance,
    }, wallet.feeRate, wallet.account, wallet.signer, wallet.provider));
});
...
exports.alkaneCreatePool = new commander_1.Command('create-pool')
    ...
    console.log(await (0, factory_1.createNewPool)(calldata, alkaneTokensToPool[0].alkaneId, alkaneTokensToPool[0].amount, alkaneTokensToPool[1].alkaneId, alkaneTokensToPool[1].amount, {
        utxos: accountSpendableTotalUtxos,
        totalAmount: accountSpendableTotalBalance,
    }, wallet.feeRate, wallet.account, wallet.signer, wallet.provider));
});
...
*/
exports.alkaneMint = new commander_1.Command('mint')
    .requiredOption('-data, --calldata <calldata>', 'op code + params to be called on a contract', (value, previous) => {
    const items = value.split(',');
    return previous ? previous.concat(items) : items;
}, [])
    ...
    console.log(await (0, pool_1.mint)(calldata, alkaneTokensToMint[0].alkaneId, alkaneTokensToMint[0].amount, alkaneTokensToMint[1].alkaneId, alkaneTokensToMint[1].amount, {
        utxos: accountSpendableTotalUtxos,
        totalAmount: accountSpendableTotalBalance,
    }, wallet.feeRate, wallet.account, wallet.signer, wallet.provider));
});
...
exports.alkaneSimulate = new commander_1.Command('simulate')
    ...
    if (options.decoder === 'pool') {
        const { AlkanesPoolSimulateDecoder, PoolOpcodes } = await Promise.resolve().then(() => tslib_1.__importStar(require('../amm/pool')));
        decoder = (result) => AlkanesPoolSimulateDecoder.decodeSimulation(result, Number(options.inputs[0]));
    }
