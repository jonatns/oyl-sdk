...
exports.alkaneSpendCommit = new AlkanesCommand('spend-commit')
    ...
    .requiredOption('-m, --method <method>', 'The method to use to spend the commit, either "reveal" or "recover".')
    .requiredOption('-c, --contract <contract>', 'Relative path to contract wasm file to deploy (e.g., "../alkanes/free_mint.wasm")')
    .option('-data, --calldata <calldata>', 'op code + params to be used when deploying a contracts', (value, previous) => {
    const items = value.split(',');
    return previous ? previous.concat(items) : items;
}, [])
    .option('-p, --provider <provider>', 'Network provider type (regtest, bitcoin)')
    ...
    if (options.contract) {
        ...
        script = Buffer.from((0, alkanes_1.p2tr_ord_reveal)((0, bip371_1.toXOnly)(tweakedPublicKey), [payload]).script);
    }
    if (options.method === 'reveal') {
        console.log(await (0, contract_1.deployReveal)({
        ...
        console.log(await (0, contract_1.recoverCommit)({
            commitTxId: options.commitTxId,
            utxos: accountUtxos,
            account: wallet.account,
            provider: wallet.provider,
