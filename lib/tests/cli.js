...
const btc = __importStar(require("../btc"));
const constants_1 = require("../shared/constants");
const collectible = __importStar(require("../collectible"));
    ...
    switch (command) {
        ...
        case 'available-utxos':
            return console.log(await (0, utxo_1.accountSpendableUtxos)({
                account: regtestAccount,
                provider: constants_1.regtestProvider,
                spendAmount: 1000,
            }));
        ...
        case 'new-btc-send':
            ...
            const vsize = (await constants_1.regtestProvider.sandshrew.bitcoindRpc.decodePSBT(signedPsbt)).tx.vsize;
        ...
        case 'new-brc-20-send':
            let successTxIds = [];
            const estimate = await (0, brc20_1.transferEstimate)({
                toAddress: networkConfig.destinationTaprootAddress,
                feeRate: 2,
                account: regtestAccount,
                provider: constants_1.regtestProvider,
            });
            const { psbt: dryCommitPsbt } = await (0, brc20_1.commit)({
                ticker: 'toyl',
                amount: 100,
                feeRate: 2,
                account: regtestAccount,
            ...
            const { psbt: finalCommitPsbt, script } = await (0, brc20_1.commit)({
                ticker: 'toyl',
                amount: 100,
                feeRate: 2,
                account: regtestAccount,
            ...
            const { txId: commitTxId } = await constants_1.regtestProvider.pushPsbt({
                psbtBase64: finalCommitSigned,
            });
            successTxIds.push(commitTxId);
            const { psbt: revealPsbt } = await (0, brc20_1.reveal)({
                feeRate: 2,
                account: regtestAccount,
                provider: constants_1.regtestProvider,
                script: script,
            ...
            const { psbt: finalRevealPsbt } = await (0, brc20_1.reveal)({
                feeRate: 2,
                account: regtestAccount,
                provider: constants_1.regtestProvider,
                script: script,
            ...
            const { txId: revealTxId } = await constants_1.regtestProvider.pushPsbt({
            ...
            await (0, utils_1.waitForTransaction)({
                txId: revealTxId,
                sandshrewBtcClient: constants_1.regtestProvider.sandshrew,
            });
            await (0, utils_1.delay)(5000);
            const { psbt: transferPsbt } = await (0, brc20_1.transfer)({
                feeRate: 2,
                account: regtestAccount,
                provider: constants_1.regtestProvider,
            ...
            const { psbt: finalTransferPsbt } = await (0, brc20_1.transfer)({
                feeRate: 2,
                account: regtestAccount,
                provider: constants_1.regtestProvider,
                revealTxId: revealTxId,
            ...
            const { txId: transferTxId } = await constants_1.regtestProvider.pushPsbt({
