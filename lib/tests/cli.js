...
async function runCLI() {
    ...
    const { _, network } = yargs_1.default.argv;
    ...
    switch (command) {
        ...
        case 'new-account':
            ...
            const all = (0, account_1.mnemonicToAccount)(process.env.MAINNET_MNEMONIC.trim(), opts);
            ...
            const vsize = (await provider.sandshrew.bitcoindRpc.decodeRawTransaction(raw.extractTransaction().toHex())).vsize;
            console.log(vsize);
            const correctFee = vsize * 20;
            const { psbt: finalPsbt, fee: finalFee } = await (0, btc_1.createTx)({
                toAddress: networkConfig.taprootAddress,
                amount: 100,
                feeRate: 20,
                fee: correctFee,
        ...
        case 'bis-test':
            return console.log(await await networkConfig.wallet.apiClient.getAllInscriptionsByAddress('tb1pstyemhl9n2hydg079rgrh8jhj9s7zdxh2g5u8apwk0c8yc9ge4eqp59l22').data);
        case 'utxo-artifacts':
