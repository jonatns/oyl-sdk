...
const opcodesHRV = ['name', 'symbol', 'totalSupply', 'cap', 'minted'];
class AlkanesRpc {
    alkanesUrl;
    constructor(url) {
        this.alkanesUrl = url;
        ...
        for (let j = 0; j < opcodes.length; j++) {
            try {
                ...
                if (result.status === 0) {
                    ...
                    alkaneData.percentageMinted = Math.floor((alkaneData.totalSupply / alkaneData.cap) * 100);
                }
            }
            catch (error) {
                console.log(error);
    ...
    async getAlkanes({ amount, startIndex = 1, }) {
        const alkaneResults = [];
