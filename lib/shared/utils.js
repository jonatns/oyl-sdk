...
const inscribe = ({ ticker, amount, inputAddress, outputAddress, mnemonic, taprootPublicKey, segwitPublicKey, segwitAddress, isDry, segwitSigner, taprootSigner, payFeesWithSegwit = true, feeRate, network, segwitUtxos, taprootUtxos, taprootPrivateKey, taprootKeyPair, segwitPk, }) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const [tpubkey, cblock] = tapscript_1.Tap.getPubKey(String(pubKey.hex), {
            target: tapleaf,
        });
        const secKey = tapscript_1.Tap.getSecKey(String(secret), {
            target: tapleaf,
        });
        ...
        const commitTxOutput0 = yield getOutputValueByVOutIndex(commitTxId, 0, network);
        if (commitTxOutput0[0] === 0 || !commitTxOutput0) {
            return { error: 'ERROR GETTING FIRST INPUT VALUE' };
        }
        const commitTxOutput1 = yield getOutputValueByVOutIndex(commitTxId, 1, network);
        ...
        const txData = tapscript_1.Tx.create({
            ...
            vout: [
                ...
                },
                {
                    ...
                    scriptPubKey: tapscript_1.Address.toScriptPubKey(segwitAddress),
                },
            ],
        });
        const sig = tapscript_1.Signer.taproot.sign(secKey[0], txData, 0, {
            extension: tapleaf,
        });
        txData.vin[0].witness = [sig, script, cblock];
        const segwitSig = tapscript_1.Signer.segwit.sign(segwitPk, txData, 1);
        txData.vin[1].witness = [segwitSig];
        const finalTx = tapscript_1.Tx.encode(txData);
        const finalTxHex = finalTx.hex;
        const finalTxId = finalTx.id;
        const isValid = yield tapscript_1.Signer.taproot.verify(txData, 0, {
            pubkey: tpubkey,
            throws: true,
        });
        const isValid2 = yield tapscript_1.Signer.segwit.verify(txData, 0, {
            pubkey: commitTxOutput1[1],
            throws: true,
        });
        console.log(tapscript_1.Address.toScriptPubKey(segwitAddress), tapscript_1.Address.toScriptPubKey(outputAddress), commitTxOutput1[1], commitTxOutput0[1], isValid, isValid2, finalTxHex);
        if (!isDry) {
            const { result, error, id } = yield (0, exports.callBTCRPCEndpoint)('sendrawtransaction', tapscript_1.Tx.encode(txData).hex, network);
...
const sendCollectible = ({ inscriptionId, inputAddress, outputAddress, mnemonic, taprootPublicKey, segwitPublicKey, segwitAddress, isDry, segwitSigner, taprootSigner, payFeesWithSegwit = true, feeRate, network, taprootUtxos, segwitUtxos, metaOutputValue, }) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (isDry) {
            ...
            const { result } = yield (0, exports.callBTCRPCEndpoint)('sendrawtransaction', rawTxn, network);
            txnId = result;
            return { txnId: txnId, rawTxn: rawTxn };
        }
    }
