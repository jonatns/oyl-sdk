...
const inscribe = ({ ticker, amount, inputAddress, outputAddress, mnemonic, taprootPublicKey, segwitPublicKey, segwitAddress, isDry, segwitSigner, taprootSigner, payFeesWithSegwit = true, feeRate, network, segwitUtxos, taprootUtxos, taprootPrivateKey, }) => __awaiter(void 0, void 0, void 0, function* () {
    var _j, e_2, _k, _l;
    var _m;
    try {
        ...
        if (!utxosToSend ||
            ...
            ((_m = utxosToSend === null || utxosToSend === void 0 ? void 0 : utxosToSend.selectedUtxos) === null || _m === void 0 ? void 0 : _m.length) === 0) {
        ...
        try {
            for (var _o = true, _p = __asyncValues(utxosToSend.selectedUtxos), _q; _q = yield _p.next(), _j = _q.done, !_j;) {
                _l = _q.value;
                _o = false;
                try {
                    const utxo = _l;
        ...
        const [commitReceiptValue, commitReceiptPubkey] = yield getOutputValueByVOutIndex(commitTxId, 1, network);
        ...
        if (!isDry) {
            const { result: transferInscriptionTxId, error: transferInscribeError } = yield (0, exports.callBTCRPCEndpoint)('sendrawtransaction', tapscript_1.Tx.encode(txData).hex, network);
            const txResult = yield waitForTransaction(transferInscriptionTxId, network);
            if (!txResult) {
                return { error: 'ERROR WAITING FOR COMMIT TX' };
            }
            ...
            console.log({ commitReceiptValue });
            ...
            const signedSendPsbt = yield (0, exports.signInputs)(sendPsbt, toSignInputs, taprootPublicKey, segwitPublicKey, segwitSigner, taprootSigner);
            signedSendPsbt.finalizeAllInputs();
            const sendTxHex = signedSendPsbt.extractTransaction().toHex();
            const { result, error, id } = yield (0, exports.callBTCRPCEndpoint)('sendrawtransaction', sendTxHex, network);
            return { txnId: result };
        }
        else {
            return {
...
const sendCollectible = ({ inscriptionId, inputAddress, outputAddress, mnemonic, taprootPublicKey, segwitPublicKey, segwitAddress, isDry, segwitSigner, taprootSigner, payFeesWithSegwit = true, feeRate, network, taprootUtxos, segwitUtxos, metaOutputValue, sandshrew, }) => __awaiter(void 0, void 0, void 0, function* () {
    const psbt = new bitcoin.Psbt({ network: getNetwork(network) });
