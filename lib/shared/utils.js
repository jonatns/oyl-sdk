...
const createInscriptionScript = (pubKey, content) => {
    ...
    return `${pubKey} OP_CHECKSIG OP_0 OP_IF ${markerHex} ${mimeTypeHex} OP_0 ${contentHex} OP_ENDIF`;
};
...
const addBtcUtxo = async ({ spendUtxos, toAddress, psbt, amount, feeRate, network, spendAddress, spendPubKey, altSpendPubKey, altSpendUtxos, fee, }) => {
    ...
    let utxosToSend = (0, buildOrdTx_1.findUtxosToCoverAmount)(spendableUtxos, amount + finalFee);
    let usingAlt = false;
    if (utxosToSend?.selectedUtxos.length > 1) {
        ...
        utxosToSend = (0, buildOrdTx_1.findUtxosToCoverAmount)(spendableUtxos, amount + finalFee);
    }
    if (!utxosToSend) {
        const unFilteredAltUtxos = await (0, exports.filterTaprootUtxos)({
            taprootUtxos: altSpendUtxos,
        });
        utxosToSend = (0, buildOrdTx_1.findUtxosToCoverAmount)(unFilteredAltUtxos, amount + finalFee);
        if (utxosToSend?.selectedUtxos.length > 1) {
            const txSize = calculateTaprootTxSize(utxosToSend.selectedUtxos.length, 0, 2);
            fee = txSize * feeRate < 250 ? 250 : txSize * feeRate;
            utxosToSend = (0, buildOrdTx_1.findUtxosToCoverAmount)(spendableUtxos, amount + finalFee);
