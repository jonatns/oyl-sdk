...
const formatInputsToSign = ({ _psbt, pubkey, taprootAddress, network, }) => __awaiter(void 0, void 0, void 0, function* () {
    var _j, e_2, _k, _l;
    var _m, _o;
    let index = 0;
    try {
...
const inscribe = ({ content, inputAddress, outputAddress, mnemonic, taprootPublicKey, segwitPublicKey, segwitAddress, isDry, segwitSigner, taprootSigner, feeRate, network, segwitUtxos, taprootUtxos, taprootPrivateKey, sandshrewBtcClient, esploraRpc, }) => __awaiter(void 0, void 0, void 0, function* () {
    var _s, e_3, _t, _u;
    var _v;
    ...
    const amountNeededForInscribe = Number(feeForCommit) + Number(feeForReveal) + inscriptionSats;
    const utxosToSend = (0, buildOrdTx_1.findUtxosToCoverAmount)(taprootUtxos, amountNeededForInscribe);
    if (!utxosToSend ||
        !utxosToSend.selectedUtxos ||
        ((_v = utxosToSend === null || utxosToSend === void 0 ? void 0 : utxosToSend.selectedUtxos) === null || _v === void 0 ? void 0 : _v.length) === 0) {
    ...
    try {
        for (var _w = true, _x = __asyncValues(utxosToSend.selectedUtxos), _y; _y = yield _x.next(), _s = _y.done, !_s;) {
            _u = _y.value;
            _w = false;
            try {
                const utxo = _u;
        ...
        try {
            if (!_w && !_s && (_t = _x.return)) yield _t.call(_x);
        }
        finally { if (e_3) throw e_3.error; }
    }
    const revealSats = feeForReveal + inscriptionSats;
    const reimbursementAmount = amountGathered - feeForCommit - feeForReveal - inscriptionSats;
    psbt.addOutput({
    ...
    const [inscriptionResult] = yield sandshrewBtcClient.bitcoindRpc.testMemPoolAccept([inscriptionTxHex]);
    if (!inscriptionResult.allowed) {
        throw new Error(inscriptionResult['reject-reason']);
    }
    let revealTxId = '';
    if (!isDry) {
        revealTxId = yield sandshrewBtcClient.bitcoindRpc.sendRawTransaction(inscriptionTxHex);
    }
    return {
        commitTx: commitTxId,
        revealTx: revealTxId,
...
const insertBtcUtxo = ({ taprootUtxos, segwitUtxos, toAddress, fromAddress, psbt, amount, useTaprootUtxos, segwitPubKey, payFeesWithSegwit, feeRate, network, }) => __awaiter(void 0, void 0, void 0, function* () {
    const nonMetaUtxos = yield (0, exports.filterTaprootUtxos)({ taprootUtxos: taprootUtxos });
    const modifiedPsbt = yield addBTCUtxo({
...
exports.insertBtcUtxo = insertBtcUtxo;
...
const addBTCUtxo = ({ taprootUtxos, segwitUtxos, toAddress, psbtTx, amount, feeRate, fromAddress, useTaprootUtxos, payFeesWithSegwit, segwitPubKey, network, }) => __awaiter(void 0, void 0, void 0, function* () {
    const txSize = calculateTaprootTxSize(2, 0, 2);
    const fee = txSize * feeRate < 200 ? 200 : txSize * feeRate;
    const utxosToSend = (0, buildOrdTx_1.findUtxosToCoverAmount)(useTaprootUtxos ? taprootUtxos : segwitUtxos, payFeesWithSegwit ? amount : amount + fee);
    if (!utxosToSend) {
        throw new Error('insufficient balance');
    }
    const amountGathered = calculateAmountGatheredUtxo(utxosToSend.selectedUtxos);
    ...
    if (payFeesWithSegwit) {
        ...
        const utxosToPayFee = (0, buildOrdTx_1.findUtxosToCoverAmount)(segwitUtxos, amount);
        if (!utxosToPayFee) {
            throw new Error('insufficient segwit balance');
        }
        const feeAmountGathered = calculateAmountGatheredUtxo(utxosToPayFee.selectedUtxos);
        ...
        for (let i = 0; i < utxosToPayFee.selectedUtxos.length; i++) {
            if ((0, buildOrdTx_1.usableUtxo)(utxosToPayFee.selectedUtxos[i], utxosToSend.selectedUtxos)) {
                psbtTx.addInput({
