...
const transferEstimate = async ({ gatheredUtxos, toAddress, feeRate, account, provider, fee, }) => {
    try {
        ...
        gatheredUtxos = (0, utils_1.findXAmountOfSats)(gatheredUtxos.utxos, Number(finalFee) + 546);
...
const commit = async ({ gatheredUtxos, ticker, amount, feeRate, account, tweakedTaprootPublicKey, provider, finalTransferFee, fee, }) => {
    try {
        ...
        if (!gatheredUtxos) {
            ...
            gatheredUtxos = (0, utils_1.findXAmountOfSats)(accountSpendableTotalUtxos, Number(finalFee) + Number(finalTransferFee));
        }
        gatheredUtxos = (0, utils_1.findXAmountOfSats)(gatheredUtxos.utxos, Number(finalFee) + Number(finalTransferFee));
        if (gatheredUtxos.utxos.length > 1) {
            ...
            gatheredUtxos = (0, utils_1.findXAmountOfSats)(gatheredUtxos.utxos, finalFee + finalTransferFee);
