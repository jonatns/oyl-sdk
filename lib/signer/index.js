...
class Signer {
    ...
    async signAllInputs({ rawPsbt, rawPsbtHex, finalize = true, allowedSighashTypes, }) {
        let unSignedPsbt;
        if (rawPsbt) {
        ...
        for (let i = 0; i < unSignedPsbt.inputCount; i++) {
            ...
            switch (true) {
                case matchingTaprootPubKey:
                    console.log('signTaprootInput', i, tweakedSigner, undefined, allowedSighashTypes);
                    unSignedPsbt.signTaprootInput(i, tweakedSigner, undefined, allowedSighashTypes);
                    if (finalize) {
                        unSignedPsbt.finalizeInput(i);
                    }
                    break;
