...
describe('Signer', () => {
    ...
    const hashedMessage = crypto_1.default
        ...
        .digest()
        .toString('base64');
    const bip322Hash = Buffer.from(bip322_js_1.BIP322.hashMessage('Hello World')).toString('base64');
    ...
    test('Should sign a message with nested segwit keypair', async () => {
        const signtaure = 'AkcwRAIgExPs5WhKRKgvKTggfwPyDjaJkOcr7/LD8h+wGhcmYBcCIBI4KGpz7tEraXQ7a9kvlXVpxvcTma+5iqWdkQ1ec5awASEDmztpS4/FteB/sGnHg8rHVPXTjD4IvtGWDjH9sd2jXCQ=';
        ...
        const verifySignature = bip322_js_1.Verifier.verifySignature(address, bip322Hash, signedMessage);
    ...
    test('Should sign a message with taproot keypair', async () => {
        const signtaure = 'AUHnVaY2a160fGETICoVNFO6I3I+q7s32JyJQk4Pc6ZATfwdFVkYGkB8r9rAncALD+9Ri5kQN/05UKk1w2176iXaAQ==';
        let signer = new _1.Signer(network, keys);
    ...
    test('Should sign a message with native segwit keypair', async () => {
        const signtaure = 'AkgwRQIhAIeskX8weJabUu5u+gI0Tnlm99/9e2i015WVbuhzzIBxAiAFy7gj1OukkgipmBRENaE4/CZ/pJXcUcsFUZ8pQIaOSgEhAzDVT9DdQgpuX402JPXzSCyuNQ951fB1O/W+75wtka88';
        let signer = new _1.Signer(network, keys);
    ...
    test('Should sign a message with legacy keypair', async () => {
        const signtaure = 'H17cVBLC+0dxMrPP32z5T+kFNPitqwbDhzjKK/o9WTZFDq/lKA9HuElUqii6GHvqzPkW8HVSJIUNs8ljxgpAgpk=';
        let signer = new _1.Signer(network, keys);
    ...
    test('Should sign a message with ecdsa keypair', async () => {
        const signtaure = 'QzeHWtTiap2cW8+7iS/0mhzDf1PUVbTmKCx0FRb8x4EDMiHLOdoUKVn3L4+S921lpaCwPXsKvF95TCealGwuhg==';
        let signer = new _1.Signer(network, keys);
        const signedMessage = await signer.signMessage({
            message,
            keypair: signer.segwitKeyPair,
        ...
        const verifySignature = signer.segwitKeyPair.verify(Buffer.from(hashedMessage, 'base64'), Buffer.from(signedMessage, 'base64'));
        expect(signedMessage).toEqual(signtaure);
        expect(verifySignature).toBe(true);
    });
