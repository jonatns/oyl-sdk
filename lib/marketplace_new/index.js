...
class NewMarketplace {
    ...
    async selectSpendAddress(offers) {
        ...
        for (let i = 0; i < this.account.spendStrategy.addressOrder.length; i++) {
            ...
            if ((0, __1.getAddressType)(address) === __1.AddressType.P2TR) {
                pubkey = (0, __1.tweakSigner)(this.signer.taprootKeyPair, {
            ...
            if (i === this.account.spendStrategy.addressOrder.length - 1) {
                throw new __1.OylTransactionError(new Error('Not enough sats available to buy marketplace offers, need  ' +
                    estimatedCost +
                    ' sats'), this.txIds);
            }
        }
    ...
    async processAllOffers(offers) {
        ...
        if (processedOffers.length < 1) {
            throw new __1.OylTransactionError(new Error('Offers  unavailable'), this.txIds);
        }
        return {
    ...
    async buyMarketPlaceOffers(pOffers) {
        ...
        if (offers.length < 1)
            throw new __1.OylTransactionError(new Error('No offers to buy'), this.txIds);
        const marketPlaceBuy = new buildMarketplaceTx_1.BuildMarketplaceTransaction({
            address: this.selectedSpendAddress,
            pubKey: this.selectedSpendPubkey,
            receiveAddress: this.receiveAddress,
        ...
        if (!preparedWallet) {
            throw new __1.OylTransactionError(new Error('Address not prepared to buy marketplace offers'), this.txIds);
        }
        ...
        if (!broadcast.allowed) {
            ...
            throw new __1.OylTransactionError(result['reject-reason'], this.txIds);
        }
    ...
    async prepareAddress(marketPlaceBuy) {
        try {
            ...
            throw new __1.OylTransactionError(new Error('An error occured while preparing address for marketplace buy'), this.txIds);
        }
    }
    ...
    async getUnspentsForAddress(address) {
        try {
            ...
            throw new __1.OylTransactionError(e, this.txIds);
        }
    }
    ...
    async buildDummyAndPaymentUtxos(orderPrice) {
        ...
        if (allUtxosWorth600.length < 2) {
            throw new __1.OylTransactionError(new Error('not enough padding utxos (600 sat) for marketplace buy'), this.txIds);
        }
        const dummyUtxos = [];
        for (let i = 0; i < 2; i++) {
            dummyUtxos.push({
        ...
        if (retrievedUtxos.length === 0) {
            throw new __1.OylTransactionError(new Error('Not enough funds to purchase this offer'), this.txIds);
        }
        const paymentUtxos = [];
...
exports.NewMarketplace = NewMarketplace;
