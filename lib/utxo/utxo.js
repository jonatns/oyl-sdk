...
function mapAlkanesById(outpointAlkanes) {
    return outpointAlkanes.reduce((acc, { token, value }) => {
...
const addressUtxos = async ({ address, provider, spendStrategy, }) => {
    ...
    for (const { utxo, txOutput, scriptPk, alkanesByOutpoint, } of processedUtxos) {
