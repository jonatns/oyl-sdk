...
class Marketplace {
    ...
    async processMultipleBuys(orders, previousOrderTxId, remainingSats, index = 1, psbtBase64s = [], psbtHexs = [], txIds = []) {
        ...
        const { psbtBase64: filledOutBase64, remainingSats: updatedRemainingSats, psbtHex: filledOutPsbtHex, } = await marketPlaceBuy.psbtMultiBuilder(previousOrderTxId, remainingSats);
        ...
        const result = await this.wallet.sandshrewBtcClient.bitcoindRpc.finalizePSBT(signedPsbt.toBase64());
    ...
    async buyMarketPlaceOffers(pOffers) {
        ...
        const { psbtBase64, remainder, psbtHex } = await marketPlaceBuy.psbtBuilder();
        ...
        const signedPsbt = await txSigner.signPsbt(tempPsbt, false);
        const result = await this.wallet.sandshrewBtcClient.bitcoindRpc.finalizePSBT(signedPsbt.toBase64());
        const [broadcast] = await this.wallet.sandshrewBtcClient.bitcoindRpc.testMemPoolAccept([result.hex]);
        ...
        const multipleBuys = await this.processMultipleBuys(offers, txId, remainingSats, 1);
        for (let i = 0; i < multipleBuys.psbtHexs.length; i++) {
            await (0, __1.timeout)(30000);
            const [broadcast] = await this.wallet.sandshrewBtcClient.bitcoindRpc.testMemPoolAccept([multipleBuys.psbtHexs[i]]);
            if (!broadcast.allowed) {
    ...
    async externalSign(options) {
        ...
        const signer = keyring.signTransaction.bind(keyring);
        const from = address;
        const addressType = (0, __1.getAddressType)(from);
        if (addressType == null)
            throw Error("Invalid Address Type");
        const tx = new __1.OGPSBTTransaction(signer, from, pubKey, addressType, this.wallet.network, feeRate);
    ...
    **/
    async checkIfOfferIsValid(offer) {
        return false;
    }
    /**
     ...
     **/
    async validateTxidInMempool(txid) {
    }
}
