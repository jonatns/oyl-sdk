...
const getDerivationPaths = (index = 0, network = bitcoin.networks.bitcoin, derivationMode) => {
...
const generateWallet = ({ mnemonic, opts, }) => {
    ...
    const paths = (0, exports.getDerivationPaths)(opts.index, opts.network, opts.derivationMode);
...
const getWalletPrivateKeys = ({ mnemonic, opts, }) => {
    ...
    const paths = (0, exports.getDerivationPaths)(options.index, options.network, opts?.derivationMode);
    const seed = bip39.mnemonicToSeedSync(mnemonic);
    const root = bip32.fromSeed(seed);
    // Legacy
    const childLegacy = root.derivePath(paths.legacy);
