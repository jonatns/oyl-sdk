...
const createPsbt = async ({ gatheredUtxos, toAddress, amount, feeRate, account, provider, fee, }) => {
    try {
        if (!gatheredUtxos.utxos?.length) {
...
const send = async ({ gatheredUtxos, toAddress, amount, feeRate, account, provider, signer, fee, }) => {
    if (!fee) {
...
exports.send = send;
const actualFee = async ({ gatheredUtxos, toAddress, amount, feeRate, account, provider, signer, }) => {
