...
exports.createRBFPsbt = void 0;
const tslib_1 = require("tslib");
const bitcoin = tslib_1.__importStar(require("bitcoinjs-lib"));
const utils_1 = require("../shared/utils");
const alkanes_1 = require("../alkanes/alkanes");
async function createRBFPsbt(txid, new_fee_rate, account, provider) {
    // 1. Fetch raw tx hex
    const txHex = await provider.sandshrew.bitcoindRpc.getRawTransaction?.(txid, 0);
    if (!txHex)
        throw new Error('Could not fetch raw transaction');
    // 2. Decode raw tx
    const tx = bitcoin.Transaction.fromHex(txHex);
    // 3. Gather all wallet addresses (taproot, segwit, etc)
    const walletAddresses = [];
    if (account.taproot?.address)
        walletAddresses.push(account.taproot.address);
    ...
    if (account.legacy?.address)
        walletAddresses.push(account.legacy.address);
    // 4. Prepare new PSBT
    ...
    for (let i = 0; i < tx.ins.length; i++) {
        ...
        if (!prevTxInfo)
            throw new Error('Could not fetch prevout tx info');
        const prevout = prevTxInfo.vout[input.index];
        if (!prevout)
            throw new Error('Prevout not found');
        ...
        if (walletAddresses.includes(prevoutAddress)) {
            ...
            await (0, alkanes_1.addInputForUtxo)(psbt, data, account, provider);
            await (0, alkanes_1.addInputForUtxo)(dummyPsbt, data, account, provider);
        }
        else {
            // Not our input, add minimal info
            const prevTxHex = await provider.sandshrew.bitcoindRpc.getRawTransaction?.(prevTxId, 0);
            const i = {
                hash: prevTxId,
                index: input.index,
...
exports.createRBFPsbt = createRBFPsbt;
