...
exports.alkaneMultiSend = exports.actualAlkaneMultiSendFee = exports.createAlkaneMultiSendPsbt = exports.actualSplitFee = exports.createSplitPsbt = exports.split = exports.actualSendFee = exports.send = exports.createSendPsbt = exports.createRevealPsbt = exports.createCommitPsbt = exports.inscribePayload = void 0;
...
const createCommitPsbt = async ({ payload, utxos, account, protostone, provider, feeRate, tweakedPublicKey, frontendFee, feeAddress, }) => {
    // Calculate fees for commit transaction
    const { fee: commitFee, deployRevealFee } = await (0, alkanes_1.actualDeployCommitFee)({
        payload,
        utxos,
        tweakedPublicKey,
        account,
    ...
    // Create commit PSBT
    const { psbt, script } = await (0, alkanes_1.createDeployCommitPsbt)({
        payload,
        utxos,
        tweakedPublicKey,
        account,
...
exports.createCommitPsbt = createCommitPsbt;
const createRevealPsbt = async ({ payload, utxos, account, protostone, provider, feeRate, tweakedPublicKey, commitTxId, commitPsbtBase64, script, }) => {
    ...
    const { fee: revealFee } = await (0, alkanes_1.actualTransactRevealFee)({
        payload,
        utxos,
        protostone,
        tweakedPublicKey,
    ...
    // Create reveal PSBT
    const { psbt } = await (0, alkanes_1.createTransactReveal)({
        payload,
        utxos,
        protostone,
        tweakedPublicKey,
...
exports.createRevealPsbt = createRevealPsbt;
const createSendPsbt = async ({ utxos, account, alkaneId, provider, toAddress, amount, feeRate, fee, }) => {
    try {
        ...
        let gatheredUtxos = (0, utils_1.findXAmountOfSats)(totalSpendableUtxos.utxos, Number(finalFee) + Number(utils_1.inscriptionSats) * 2);
        if (gatheredUtxos.utxos.length > 1) {
            ...
            gatheredUtxos = (0, utils_1.findXAmountOfSats)(totalSpendableUtxos.utxos, Number(finalFee) + Number(utils_1.inscriptionSats) * 2);
        }
        ...
        const protostone = (0, index_js_1.encodeRunestoneProtostone)({
            protostones: [
                index_js_1.ProtoStone.message({
                    ...
                    pointer: 0,
                    refundPointer: 0,
                    calldata: (0, index_js_1.encipher)([BigInt(2), BigInt(0), BigInt(77)]),
                }),
            ],
        }).encodedRunestone;
        psbt.addOutput({
...
const createAlkaneMultiSendPsbt = async ({ sends, alkaneId, utxos, account, provider, feeRate, fee = 0, }) => {
    try {
        ...
        if (protostone.length > 80) {
            throw new Error(`OP_RETURN script size is too large: ${protostone.length} bytes`);
        }
...
exports.createAlkaneMultiSendPsbt = createAlkaneMultiSendPsbt;
const actualAlkaneMultiSendFee = async ({ sends, alkaneId, utxos, account, provider, feeRate, }) => {
    const { psbt } = await (0, exports.createAlkaneMultiSendPsbt)({
        sends,
        alkaneId,
        utxos,
        account,
    ...
    const { psbt: finalPsbt } = await (0, exports.createAlkaneMultiSendPsbt)({
        sends,
        alkaneId,
        utxos,
        account,
...
exports.actualAlkaneMultiSendFee = actualAlkaneMultiSendFee;
const alkaneMultiSend = async ({ sends, alkaneId, utxos, account, provider, signer, feeRate, }) => {
    const { fee } = await (0, exports.actualAlkaneMultiSendFee)({
        sends,
        alkaneId,
        utxos,
        account,
