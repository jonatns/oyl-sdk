...
const lib_1 = require("alkanes/lib");
...
const createExecutePsbt = async ({ gatheredUtxos, account, calldata, provider, feeRate, fee = 0, }) => {
    try {
        ...
        const script = (0, lib_1.encodeRunestoneProtostone)({
            protostones: [
                lib_1.ProtoStone.message({
                    protocolTag: 1n,
                    edicts: [],
                    pointer: 0,
                    refundPointer: 0,
                    calldata: (0, lib_1.encipher)(calldata),
                }),
            ],
        }).encodedRunestone;
        psbt.addOutput({
...
const createDeployCommit = async ({ payload, gatheredUtxos, tweakedTaprootKeyPair, account, provider, feeRate, fee, }) => {
    try {
        ...
        let psbt = new bitcoin.Psbt({ network: provider.network });
        const script = Buffer.from((0, lib_1.p2tr_ord_reveal)((0, bip371_1.toXOnly)(tweakedTaprootKeyPair.publicKey), [payload])
            .script);
...
const createDeployReveal = async ({ createReserveNumber, receiverAddress, script, feeRate, tweakedTaprootKeyPair, provider, fee = 0, commitTxId, }) => {
    try {
        ...
        const protostone = (0, lib_1.encodeRunestoneProtostone)({
            protostones: [
                lib_1.ProtoStone.message({
                    protocolTag: 1n,
                    edicts: [],
