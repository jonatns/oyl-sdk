...
const createExecutePsbt = async ({ alkaneUtxos, gatheredUtxos, account, protostone, provider, feeRate, fee = 0, }) => {
    try {
        ...
        gatheredUtxos = (0, utils_1.findXAmountOfSats)(originalGatheredUtxos.utxos, Number(finalFee) + 1092);
        ...
        const changeAmount = gatheredUtxos.totalAmount + alkaneUtxos.totalSatoshis - finalFee - 1092;
        psbt.addOutput({
...
const actualExecuteFee = async ({ gatheredUtxos, account, protostone, provider, feeRate, signer, alkaneUtxos, }) => {
    if (!feeRate) {
        feeRate = (await provider.esplora.getFeeEstimates())['1'];
    }
