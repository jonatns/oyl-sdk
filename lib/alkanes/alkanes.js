...
const createTransactReveal = async ({ payload, alkanesUtxos, utxos, protostone, receiverAddress, script, feeRate, tweakedPublicKey, provider, fee = 0, commitTxId, account, }) => {
    try {
        ...
        if (commitTxOutput.value < revealTxFee + wasmDeploySize + 546) {
            gatheredUtxos = (0, utils_1.findXAmountOfSats)([...utxos], revealTxFee + wasmDeploySize + 546 - commitTxOutput.value);
        }
        ...
        const totalAlkanesAmount = alkanesUtxos
            ? alkanesUtxos.reduce((acc, utxo) => acc + utxo.satoshis, 0)
            : 0;
        const change = commitTxOutput.value + totalAlkanesAmount + gatheredUtxos.totalAmount - revealTxFee - wasmDeploySize - 546;
        if (change > 546) {
            psbt.addOutput({
                value: change,
                address: receiverAddress,
