...
const executeFallbackToWitnessProxy = async ({ alkanesUtxos, utxos, account, calldata, provider, feeRate, signer, frontendFee, feeAddress, witnessProxy, frbtcWrapAmount, }) => {
    let frbtcWrapPsbt;
    ...
    if (frbtcWrapAmount) {
        ...
        console.log("utxos before filter", utxos);
        remainingUtxos = utxos.filter(utxo => !spentUtxos.some(spent => spent.txId === utxo.txId && spent.outputIndex === Number(utxo.outputIndex)));
        console.log("utxos after filter", remainingUtxos);
        if (alkanesUtxos) {
            console.log("alkanesUtxos before filter", alkanesUtxos);
            ...
            console.log("alkanesUtxos after filter", remainingAlkanesUtxos);
        }
    }
...
const wrapBtcNoSigning = async ({ alkanesUtxos, utxos, account, provider, feeRate, wrapAmount, }) => {
