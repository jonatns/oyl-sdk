...
//   }
// }
const createExecutePsbt = async ({ gatheredUtxos, account, calldata, provider, feeRate, fee, }) => {
    try {
        const originalGatheredUtxos = gatheredUtxos;
...
const actualExecuteFee = async ({ gatheredUtxos, account, calldata, provider, feeRate, }) => {
    if (!feeRate) {
        feeRate = (await provider.esplora.getFeeEstimates())['1'];
    }
    const { psbtHex } = await (0, exports.createExecutePsbt)({
        gatheredUtxos,
        account,
        calldata,
        provider,
    ...
    const vsize = (await provider.sandshrew.bitcoindRpc.testMemPoolAccept([psbtHex]))[0].vsize;
    const correctFee = vsize * feeRate;
    const { psbtHex: finalPsbtHex } = await (0, exports.createExecutePsbt)({
        gatheredUtxos,
        account,
        calldata,
        provider,
