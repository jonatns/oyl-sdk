...
const executeFallbackToWitnessProxy = async ({ alkanesUtxos, utxos, account, calldata, provider, feeRate, signer, frontendFee, feeAddress, witnessProxy, }) => {
...
const wrapBtc = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, signer, wrapAmount, }) => {
...
const inscribePayloadBulk = async ({ alkanesUtxos, payload, utxos, account, protostone, provider, feeRate, signer, }) => {
    const tweakedTaprootKeyPair = (0, utils_1.tweakSigner)(signer.taprootKeyPair, {
        network: provider.network,
    });
    const tweakedPublicKey = tweakedTaprootKeyPair.publicKey.toString('hex');
