...
const createUnwrapBtcPsbt = async ({ utxos, account, provider, feeRate, fee = 0, unwrapAmount, alkaneUtxos, }) => {
    try {
        ...
        const calldata = [32n, 0n, 78n, BigInt(dustOutputIndex), unwrapAmount];
        const protostones = [];
...
const getRemainingUtxosAfterPsbt = ({ psbt, alkanesUtxos, utxos, account, network }) => {
    ...
    if (alkanesUtxos) {
        remainingAlkanesUtxos = alkanesUtxos.filter(utxo => !spentUtxos.some(spent => spent.txId === utxo.txId && spent.outputIndex === Number(utxo.outputIndex)));
    }
