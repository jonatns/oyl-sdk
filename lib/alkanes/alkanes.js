...
const actualDeployCommitFee = async ({ payload, tweakedPublicKey, utxos, account, provider, feeRate, protostone, frontendFee, feeAddress, }) => {
    if (!feeRate) {
...
const createDeployCommitPsbt = async ({ payload, utxos, tweakedPublicKey, account, provider, feeRate, fee, deployRevealFee, frontendFee, feeAddress, }) => {
    try {
        let alkanesAddress;
        let alkanesPubkey;
        if (account.taproot) {
...
const actualTransactRevealFee = async ({ payload, alkanesUtxos, utxos, protostone, tweakedPublicKey, commitTxId, commitPsbt, receiverAddress, script, provider, feeRate, account, frbtcWrapPsbt, }) => {
    if (!feeRate) {
        feeRate = (await provider.esplora.getFeeEstimates())['1'];
    }
...
const createTransactReveal = async ({ payload, alkanesUtxos, utxos, protostone, receiverAddress, script, feeRate, tweakedPublicKey, provider, fee = 0, commitTxId, commitPsbt, account, frbtcWrapPsbt, }) => {
    try {
        if (!feeRate) {
            feeRate = (await provider.esplora.getFeeEstimates())['1'];
        }
