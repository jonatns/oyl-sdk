...
const createSendPsbt = async ({ gatheredUtxos, account, alkaneId, provider, toAddress, amount, feeRate, fee, }) => {
    try {
        ...
        const { alkaneUtxos } = await (0, exports.findAlkaneUtxos)({
...
const findAlkaneUtxos = async ({ address, greatestToLeast, provider, 
//runeId,
targetNumberOfRunes, }) => {
    const alkaneUtxos = [];
    ...
    console.log(res.outpoints);
    // if (greatestToLeast) {
    //   res?.outpoints.sort((a, b) => b.runes - a.satoshis)
    // } else {
    //   res?.outpoints.sort((a, b) => a.satoshis - b.satoshis)
    // }
    let runeTotalSatoshis = 0;
    let runeTotalAmount = 0;
    let divisibility;
    // for (const rune of res.outpoints) {
    ...
    // }
    //}
    return { alkaneUtxos, runeTotalSatoshis, divisibility };
};
...
const send = async ({ gatheredUtxos, toAddress, amount, alkaneId, inscriptionAddress, feeRate, account, provider, signer, }) => {
...
(0, exports.findAlkaneUtxos)({
    address: '',
    greatestToLeast: true,
    provider: cli_1.defaultProvider.regtest,
    targetNumberOfRunes: 10,
