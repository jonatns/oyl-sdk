...
const actualDeployCommitFee = async ({ payload, tweakedPublicKey, utxos, account, provider, feeRate, protostone, }) => {
    ...
    const originalGetOutputValue = utils_1.getOutputValueByVOutIndex;
    utils_1.getOutputValueByVOutIndex = async ({ txId, vOut, esploraRpc, }) => {
        return { value: finalFee + wasmDeploySize + 546, script: '' };
    };
    const { fee: deployRevealFee } = await (0, exports.actualTransactRevealFee)({
        payload,
        utxos,
        protostone,
        tweakedPublicKey,
    ...
    utils_1.getOutputValueByVOutIndex = originalGetOutputValue;
...
const createDeployCommitPsbt = async ({ payload, utxos, tweakedPublicKey, account, provider, feeRate, fee, deployRevealFee, }) => {
    try {
        ...
        const revealTxFee = deployRevealFee ? deployRevealFee + utils_1.inscriptionSats : finalFee + wasmDeploySize + utils_1.inscriptionSats;
        ...
        const changeAmount = gatheredUtxos.totalAmount - (finalFee + revealTxFee);
        psbt.addOutput({
            address: account[account.spendStrategy.changeAddress].address,
            value: changeAmount,
        });
