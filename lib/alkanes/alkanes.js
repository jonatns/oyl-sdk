...
const unwrapBtc = async ({ utxos, account, provider, feeRate, signer, unwrapAmount, alkaneUtxos, }) => {
    const { fee, vsize } = await (0, exports.actualUnwrapBtcFee)({
        utxos,
        account,
        provider,
        feeRate,
...
const executeFallbackToWitnessProxy = async ({ alkanesUtxos, utxos, account, calldata, provider, feeRate, signer, frontendFee, feeAddress, witnessProxy, frbtcWrapAmount, addDieselMint, }) => {
    let frbtcWrapPsbt;
    ...
    if (frbtcWrapAmount) {
        ...
        remainingUtxos = utxos.filter(utxo => !spentUtxos.some(spent => spent.txId === utxo.txId && spent.outputIndex === Number(utxo.outputIndex)));
        remainingUtxos.push({
            txId: (0, utils_1.getUnfinalizedPsbtTxId)(frbtcWrapPsbt),
            outputIndex: frbtcWrapPsbt.txOutputs.length - 1,
            satoshis: frbtcWrapPsbt.txOutputs[frbtcWrapPsbt.txOutputs.length - 1].value,
            scriptPk: frbtcWrapPsbt.txOutputs[frbtcWrapPsbt.txOutputs.length - 1].script.toString('hex'),
...
const execute = async ({ alkanesUtxos, utxos, account, protostone, provider, feeRate, signer, frontendFee, feeAddress, frbtcWrapPsbt, }) => {
    const { fee } = await (0, exports.actualExecuteFee)({
        alkanesUtxos,
        frontendFee,
        feeAddress,
