...
const createExecutePsbt = async ({ gatheredUtxos, account, calldata, provider, feeRate, fee = 0, }) => {
    try {
        ...
        const script = (0, index_1.encodeRunestoneProtostone)({
            protostones: [
...
const createDeployReveal = async ({ callData, receiverAddress, script, feeRate, tweakedTaprootKeyPair, provider, fee = 0, commitTxId, }) => {
    try {
        if (!feeRate) {
