...
const recoverCommit = async ({ commitTxId, utxos, account, provider, feeRate, signer, script, }) => {
    const tweakedTaprootKeyPair = (0, utils_1.tweakSigner)(signer.taprootKeyPair, {
        network: provider.network,
    });
    ...
    const commitAddress = bitcoin.payments.p2tr({
        internalPubkey: (0, bip371_1.toXOnly)(tweakedPublicKey),
        scriptTree,
        network: provider.network,
    }).address;
    ...
    const voutIndex = commitTx.vout.findIndex((vout) => vout.scriptpubkey_address === commitAddress);
    if (voutIndex === -1) {
    ...
    psbt.addInput({
        ...
        tapInternalKey: (0, bip371_1.toXOnly)(tweakedPublicKey),
    });
