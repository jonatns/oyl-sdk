...
const contractDeployment = async ({ payload, gatheredUtxos, account, callData, provider, feeRate, signer, }) => {
...
const actualDeployRevealFee = async ({ callData, tweakedTaprootKeyPair, commitTxId, receiverAddress, script, provider, feeRate, }) => {
    if (!feeRate) {
        feeRate = (await provider.esplora.getFeeEstimates())['1'];
    }
...
const deployReveal = async ({ callData, commitTxId, script, account, provider, feeRate, signer, }) => {
