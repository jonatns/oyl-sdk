...
export class AccountManager {
    ...
    constructor(options) {
        ...
        this.taprootKeyring = new HdKeyring({ mnemonic: this.mnemonic, hdPath: this.taprootPath, activeIndexes: this.activeIndexes })
        this.segwitKeyring = new HdKeyring({ mnemonic: this.mnemonic, hdPath: this.segwitPath, activeIndexes: this.activeIndexes })
    }
    ...
    async initializeAccounts(): Promise<oylAccounts> {
        ...
        taprootAddresses.push(publicKeyToAddress(taprootAcccounts[0], AddressType.P2TR)!);
        segwitAddresses.push(publicKeyToAddress(segwitAccounts[0], AddressType.P2WPKH)!)
    ...
    async recoverAccounts(): Promise<oylAccounts> {
        ...
        while (i < taprootAcccounts.length) {
            taprootAddresses.push(publicKeyToAddress(taprootAcccounts[i], AddressType.P2TR)!);
            segwitAddresses.push(publicKeyToAddress(segwitAccounts[i], AddressType.P2WPKH)!)
            i++
        }
        const ret: oylAccounts = {
            taproot: {
