...
export class Aggregator {
  ...
  async findBestAndClosestMatches(offers, targetAmount, marketPrice) {
    ...
    const solutions = await this.findSolutionsWithinRange(
      costs,
      amounts,
      0,
      Math.max(...amounts)
    ...
    let bestAveragePrice =
      bestPriceSolution.minCost /
      this.sumAmounts(bestPriceSolution.selectedIndices, offers)

    for (const solution of solutions) {
      let averagePrice =
        solution.minCost / this.sumAmounts(solution.selectedIndices, offers)
  ...
  async findSolutionsWithinRange(
    costs,
    weights,
    minWeightLimit,
    maxWeightLimit
