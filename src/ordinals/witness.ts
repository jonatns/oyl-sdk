...
export function witnessScriptBuilder(options: WitnessScriptOptions) {
  ...
  if (!options.mediaType || !options.mediaContent || !options.pubKeyHex) {
    throw new Error(
      'Missing required parameters to build witness script. mediaType, mediaContent & pubkeyHex are required'
    )
  }
...
function processDataBytes(data) {
  if (data.byteLength > maximumScriptBytes) {
    throw new Error(
      'Data above the maximum script bytes. Try splitting into smaller chunks first'
    )
  }
